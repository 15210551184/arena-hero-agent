...
class DashboardSnapshotTests(unittest.TestCase):
    def test_default_record_caps(self) -> None:
        self.assertEqual(DashboardMemory().max_rows, 8000)
...
class CoreFarmerTests(unittest.TestCase):
    ...
    def test_cargo_worker_queues_near_core_instead_of_waiting_far_away(self) -> None:
        turn = make_turn(
            resources=0,
            units=[
                unit(WORKER_1, "WORKER", (0, 0), cargo=1),
    ...
    def test_runtime_config_rejects_invalid_values_without_mutation(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with self.assertRaises(ValueError):
                store.apply(
                    {"worker_target": 40, "max_fleet_units": 40}
                )
...
class EventLoopTests(unittest.TestCase):
    ...
    def test_max_fleet_units_cli_flag_parses(self) -> None:
        parser = build_parser()
        self.assertEqual(parser.parse_args([]).max_fleet_units, 34)
        self.assertEqual(
